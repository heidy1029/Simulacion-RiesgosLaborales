...
export default function Table({ size, table, wordlist, setWordlist, setListSopa, setWinner, debugMode, windowSize, gameOver, resetGame }) {
  ...
  const [tableWD, setTableWD] = useState(0);
  ...
  useEffect(() => {
    setTableWD(mainRef.current.offsetWidth);
    setTimeout(() => {
      setBoundary();      
    }, 1000);
  ...
  const onSelectionEnd = () => {
    ...
    if (match && !match.found) {
      ...
      selection.forEach((cell) => {
        let letter = document.getElementById(`td${cell.x}${cell.y}`);
    ...
    let tableLetters = document.getElementById('table-wrapper').querySelectorAll('.selected');
    tableLetters.forEach((letter) => {
      letter.classList.remove('selected');
    });
    setSelection([]);
  };
        ...
        <tr key={'tr' + i}>
          {row.map((letter, j) => {
            return (
              ...
              >
                <p id={`td${i}${j}`} className={debugMode ? 'red-border letter' : 'letter'}>{letter}</p>
  ...
  return (
    <div id='table-wrapper' className='round-corner' {...{ onTouchMove, onTouchStart, onMouseDown }}>
      <table className='table touch-action-none' ref={mainRef}>
        <tbody style={{"--cell-size":Math.floor(tableWD/size[0])+'px', width:tableWD+'px',height:tableWD+'px'}}>{wordSearchTable}</tbody>
      </table>
      {/* {selectionHighlight} */}
      {/* {highlightedWords} */}
    </div>
